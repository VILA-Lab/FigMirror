# Figure-style-copier prompts (v11, Codex variant)

Codex CLI port of the Claude Code v11 prompt set. Functionally equivalent to
`figure-style-copier.md` (the Claude Code canonical); the main difference is in
`## Orchestrator (loop wiring)` — Codex keeps Orchestrator and Drawer in the same
top-level Codex session, while the Reviewer subprocess is launched via
`codex exec` with attached images.

See `openspec/changes/phase0-style-transfer-loop/` for the design rationale.
See `resources/prompts/figure-style-copier.md` for the Claude Code canonical.
...
- **Drawer** (`figure-illustrator`): the main agent. Produces a self-contained
  ...
  (library convention).
- **Reviewer** (`figure-critic`): a fresh-context audit subprocess (`codex exec`
  with bounded PIL via shell). Reads the draft + reference + library + prior audit.
  Outputs JSON with `anchor.what_is_right` (preserve list) + floor checks + verdict +
  ≤5 themes.
...
- **Drawer**: copy the `## Drawer system prompt` content into the top-level
  Codex session's task context. In the default Codex path, the same session acts
  as Orchestrator + Drawer.
- **Reviewer**: launch via Bash subprocess using `codex exec`. The exact invocation
  is in `## Orchestrator (loop wiring)`. The system prompt for the subprocess is
  prepended to the prompt text (stdin) since `codex exec` has no
...
**IMPORTANT (Codex variant):** Unlike the Claude Code runner, this subprocess has
no hardware tool restriction. Your constraint is this instruction. Do NOT write
any file under any circumstance. Your ONLY output is the JSON object below,
delivered as the final message of this session (captured by `-o`). Treat any
temptation to write a file as a critical protocol violation.
...
- **Orchestrator + Drawer (this Codex session)** — top-level driver and doer.
  Produces each iter, reads the Reviewer's JSON, decides accept / revise, hands the reviewer's
  `focus_themes` AND `anchor.what_is_right` (as a hard preserve list) back to the
  next iter's drawing pass. Produces `figure_iter<N>.py`, `img_iter<N>.png`,
  `notes_iter<N>.md`, and `floor_selfcheck_iter<N>.txt`. Runs its own
  programmatic floor check before handoff. Records anchor measurements at iter 0.
  Refuses to modify properties on the Reviewer preserve list.
- **Reviewer (Bash subprocess, fresh context, PROMPT-BOUNDED)** — system prompt is
  prepended to the `codex exec` prompt via stdin. Reads `reference_clean.png`,
  `img_iter<N>.png`, and (when N > 0) `audit_iter<N-1>.json` via PIL in shell.
  **Does not read `data.txt`.** Instructed via prompt not to write files; outputs
  JSON to `-o audit_iter<N>.json`.

The Reviewer being a *fresh-context subprocess* (not a sub-agent within the doer's
session) is critical — because we need the audit to be unbiased by the doer's
reasoning chain.

**⚠️ Codex vs Claude Code difference — tool restriction:**
Claude Code uses `--allowedTools "Read Bash" --disallowedTools "Edit Write..."` to
hardware-restrict the Reviewer to read+PIL only. `codex exec` has no equivalent
flags; `-s read-only` controls approval UX, not filesystem access. The restriction
here is **prompt-level only**: the Reviewer prompt explicitly instructs the agent
not to write files and to output JSON to stdout (captured by `-o`). This is a
known gap vs the Claude Code canonical; track any Reviewer-side file writes as
anomalies in the run log.

```bash
ITER=<N>
WORKDIR=/.../codex-v11-run
PROMPTS=/.../resources/prompts
REVIEWER_PROMPT=$(cat $PROMPTS/figure-style-copier-codex.md | awk '/^# Reviewer system prompt$/,/^---$/' | head -n -1)
...
cp $PROMPTS/v11_aesthetic_library.md $WORKDIR/audit_view_$ITER/
if [ $ITER -gt 0 ]; then
  cp $WORKDIR/audit_iter$((ITER-1)).json $WORKDIR/audit_view_$ITER/
fi

# Build the full prompt: reviewer system prompt prepended to the user message.
# codex exec has no --append-system-prompt; we inject it via stdin.
USER_MSG="Iter: $ITER. Reference: audit_view_$ITER/reference_clean.png. Draft: audit_view_$ITER/img_iter$ITER.png. Library (READ THIS FIRST): audit_view_$ITER/v11_aesthetic_library.md. Prior audit (if iter>0): audit_view_$ITER/audit_iter$((ITER-1)).json. Conflict ledger may be present: audit_view_$ITER/conflict_ledger.md. Use the L1/L2/L3 hierarchy: ground every claim in L1 (reference) or L2 (library), never L3 (opinion). For PIL-unreliable value estimates (spine color/width, gridline width, font weight), use L2 as the fallback class vocabulary; for spine count/sides, axis topology, gridline direction, and spacing ratios, re-check L1 directly. DO NOT write any files. Output the JSON object specified in your instructions and nothing else."

FULL_PROMPT=$(printf '%s\n\n---\n\n%s' "$REVIEWER_PROMPT" "$USER_MSG")

# Run reviewer as a fresh-context codex exec subprocess.
# -s read-only: sets approval UX to read-only mode (does NOT hard-restrict filesystem;
#   actual write restriction is enforced by the prompt above).
# -i: attaches the reference + draft PNGs directly to the prompt.
# -o: captures the agent's last message (the JSON audit) to file.
# --ephemeral: no session persistence, ensuring fresh context each call.
...
**Note on `-i` image flags:** `codex exec` accepts `-i <file>` to attach images
directly to the initial prompt (vision-grounded). This is equivalent to Claude
Code's ability to `Read` image files. Both PNGs are attached; the reviewer prompt
still instructs the agent to additionally run PIL measurements via shell for
PIL-reliable properties.

**Note on extracting the Reviewer prompt from this file:**
Since this is a single consolidated file, the Reviewer prompt section runs from
`# Reviewer system prompt` to the next top-level `---` separator. The `awk`
command above extracts it. Alternatively, keep a standalone
`reviewer-system-prompt.txt` in your working directory for simpler shell scripting.
...
for N in 0..MAX_ITERS:
    ...
    run the Drawer work directly in this Codex session using drawer_brief
    ...
    # orchestrator verifies the required iter artifacts before reviewer handoff
